...
class SNEMockSemanticMatches  extends SpecialPage {
	
	function __construct() {
		parent::__construct( SNEUtil::getSpecialPageLocal('MockSemanticMatches') , '', true);
	...
	function execute($par) {
		...
		// show title
		$this->setHeaders();
		$this->returntitle = Title::makeTitle(NS_SPECIAL, SNEUtil::getSpecialPageLocal('MockSemanticMatches'));
		
		$wgOut->setPagetitle('MockSemanticMatches');
		
		$html = '<h1>List of missing property values for the Wiki page "<i>Nigeria</i>"</h1>';
		$html .= "<table width='100%' border='1'><tr><th>Value missing for property</th><th>Value requested by</th></tr>";
		$html .= '<tr><td>Area</td><td><a href="http://www.fzi.de">6 Queries on 3 pages</a></td></tr>';
